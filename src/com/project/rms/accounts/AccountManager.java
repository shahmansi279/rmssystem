...
public class AccountManager {
	...
	// This method add members to Member array members1
	{
		...
		if (!members1.isEmpty()) {
			...
			// cs = new
			// Customer(m.nMemberID,m.memFname,m.memLname,m.memEmail,m.memPhone,m.memAddress1,m.memAddress2,m.memCity,m.getMemState(),m.memZipcode,m.memPrefnotfn,m.memPrefpmt,m.memCreditcard,m.memPrimeflag,m.memRole,m.memType);
...

	{
		...
		for (int i = 0; i < AccountClient.members1.size() && !retflag; i++) {
			...
			// System.out.println("Size of the list after retrieve operation is
			// "+ AccountClient.members1.size());

		}
		/*
		 * if (retflag) System.out.println(
		 * "Member has been retrieved successfully!"); else System.out.println(
		 * "Member not found.. Pl try again..!");
		 */
		return null;
